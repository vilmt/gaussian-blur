...
Since the Gaussian kernel is separable, the blur is implemented in two passes, reducing the number of per-pixel texture lookups from (2R+1)² to 2(2R+1) for kernel radius R.
...
Furthermore, since the linear filter implemented on graphics hardware can be manipulated to return two texels at once, the number of texture lookups is further reduced from 2(2R+1) to 2(R+1). This technique is detailed in [this blog](https://lisyarus.github.io/blog/posts/compute-blur.html).
